...
# ⚠️ અહીં સ્ટેપ ૧ માંથી કોપી કરેલી "Sheet2" ની નવી લિંક પેસ્ટ કરો
AUTH_SHEET_CSV_URL = "import streamlit as st
import re
from PIL import Image
import pandas as pd
from google import genai
...
def load_allowed_numbers(url):
    try:
        df = pd.read_csv(url)
        # શીટની પહેલી કોલમમાંથી ડેટા ક્લીન કરીને રીડ કરવો
        first_col = df.columns[0]
        numbers = df[first_col].dropna().astype(str).str.strip().tolist()
        # ફ્લોટ પોઈન્ટ્સ (જેમ કે 9427488485.0) હટાવવા માટેનું લોજિક
...
# લાઈવ ગુગલ શીટ વેરિફિકેશન ઇન્ટરફેસ
if not st.session_state['user_authenticated']:
    st.markdown("<div class='login-card'>", unsafe_allow_html=True)
    st.markdown("### 🔐 Student Access Verification")
    input_phone = st.text_input("કૃпа કરીને તમારો રજિસ્ટર્ડ મોબાઈલ નંબર દાખલ કરો:", max_chars=10, placeholder="10-digit mobile number")
    ...
    if st.button("Verify Number ➔", use_container_width=True, type="primary"):
        ...
        if cleaned_phone in allowed_numbers:
            ...
            st.error("❌ આ મોબાઈલ નંબર જ્ઞાન લાઈવની રજિસ્ટર્ડ લિસ્ટમાં ઉપલબ્ધ નથી! કૃપા કરીને સાચો નંબર નાખો અથવા એકેડમીનો સંપર્ક કરો.")
