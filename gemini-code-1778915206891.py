...
# -----------------------------------------------------
# ૧. પેજ સેટિંગ (Rebranded to Gyan Academy)
# -----------------------------------------------------
...
def load_questions(url):
    fallback_questions = {
        "સંપૂર્ણ પેપર (૧૦0 ગુણ)": ["૧. આખું TAT મેઈન્સ પેપર (તમામ પ્રશ્નો)", "મારો પોતાનો પ્રશ્ન (Custom)"],
    ...
    try:
        df = pd.read_csv(url)
        q_dict = {"સંપૂર્ણ પેપર (૧૦0 ગુણ)": ["૧. આખું TAT મેઈન્સ પેપર (તમામ પ્રશ્નો)", "મારો પોતાનો પ્રશ્ન (Custom)"]}
...
<style>
    ...
    * { 
    ...
    /* Main Background Layout styling */
    .stApp {
        background-color: #0f1117;
        color: #e8eaf0;
    }
    
    /* Beautiful Dashboard Headers */
    .tat-title { 
        color: #4f8ef7; 
        text-align: center; 
        font-size: 32px; 
...
# -----------------------------------------------------
# ૫. HTML રિપોર્ટ બનાવવાનું ફંક્શન (Updated branding)
# -----------------------------------------------------
        ...
        if not st.session_state['user_name'] or not has_patel:
            ...
            with st.spinner("⏳ પેપરનું ડીપ ચેકિંગ ચાલુ છે..."):
                try:
                    ...
                    # System Prompt completely rebranded to Gyan Academy setup
                    prompt = f"""
